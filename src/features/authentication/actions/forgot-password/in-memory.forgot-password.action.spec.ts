...
describe('in memory forgot-password action', (): void => {
  ...
  it('should not set reset password code to unknown account', async (): Promise<void> => {
    ...
    await expect(firstValueFrom(inMemoryForgotPasswordAction(accounts)('unknown@taxi-gestion.com', '7868965'))).rejects.toEqual(
      new UnknownAccountError('unknown@taxi-gestion.com')
    );
  });
});
